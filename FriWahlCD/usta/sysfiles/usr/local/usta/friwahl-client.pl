...
$semester = "Wintersemester 2011/12"
  ;	# notwendig für die Überprüfung von Studienbescheinigungen
...
sub ask_bibNum {
	...
	if ( $m eq "1" ) {
		...
		{
			dialog_message( "Fehleingabe",
				"Bibliotheksnummer besteht aus 12 Ziffern ", "--beep" );
			return;
		}
...
sub verify_cert {
	my ( $rv, $verid ) = sigchild_wrapper(
		sub {
			dialog_input( "Verifikation von Bescheinigungen",
				"Verifikationsschlüssel", "--max-input", 12 );
		}
	);
	if ( $rv != 0 ) {
	...
	if ( length($verid) != 12 ) {
		dialog_message( "Fehler",
			"Verifikationsschlüssel muss aus 12 Zeichen bestehen\n" );
		return -1;
	}
...
"https://zvwgate.zvw.uni-karlsruhe.de/qispos/servlet/de.his.servlet.RequestDispatcherServlet?state=verify"
	);
	...
	if ( $pos != -1 ) {
		...
		dialog_message( "KVV-Bescheinigung",
				"Name: "
			  . $name
			  . "\nFriCard Nr: "
			  . $fricard . "\n"
		...
		if ($valid) {
			dialog_message( "Anmerkung",
"KVV-Bescheinigungen sind nur in Verbindung mit einem Studentenausweis gültig, da die Richtigkeit der Matrikelnummer sichergestellt werden muss!"
			);
		}
	...
	if ( $pos != -1 ) {
		...
		dialog_message( "Studienbescheinigung",
				"Name: "
			  . $name . "\n"
			  . ( $valid ? "--- gültig ---" : "--- UNGÜLTIG ---" ) );
		...
		if ($valid) {
			($rv) =
			  dialog_yesno( "",
				"Identität mit einem Lichtbildausweis geprüft?",
				"--defaultno" );
			return $rv ? -1 : 0;
